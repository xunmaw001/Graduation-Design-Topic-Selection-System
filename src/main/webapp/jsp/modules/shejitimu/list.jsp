...
<!--/Pre Loader -->
        ...
        <div id="content">
            ...
            <div class="container mt-0">
                <div class="row breadcrumb-bar">
                    <div class="col-md-6">
                        <h3 class="block-title">设计题目管理</h3>
                    </div>
                    <div class="col-md-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item">设计题目管理</li>
                            <li class="breadcrumb-item active">设计题目列表</li>
                        </ol>
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="widget-area-2 lochana-box-shadow">
                            <h3 class="widget-title">设计题目列表</h3>
                            <div class="table-responsive mb-3">
                                <div class="col-sm-12">
                                    ...
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        项目类型
                                        <div class="layui-input-inline">
                                            <select name="shejitimuTypesSelectSearch"  style="width: 150px;" id="shejitimuTypesSelectSearch" class="form-control form-control-sm"
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <input type="hidden" id="topiaoId">
                            <select id="toupiao" class="form-control">
                                    <option value="zhancheng">赞成票</option>
                                    <option value="fandui">反对票</option>
                            </select>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                            <button type="button" class="btn btn-primary" onclick="topiao()">提交</button>
                        </div>
                    </div><!-- /.modal-content -->
                </div><!-- /.modal -->
            </div>
    ...
    <script>
        ...
        function setDataRow(item, number) {
            ...
            //项目类型
            var shejitimuTypesCell = document.createElement('td');
            shejitimuTypesCell.innerHTML = item.shejitimuValue;
            row.appendChild(shejitimuTypesCell);
            ...
            row.appendChild(zhuangtaiTypesCell);
                ...
                //项目内容
            var shejitimuContentCell = document.createElement('td');
            //如果图文详情包含图片
            if (item.shejitimuContent && item.shejitimuContent.indexOf('img') != -1) {
                //暂时只考虑图片+文字 和 文字+图片的情况
                var beginIndex = item.shejitimuContent.indexOf('<img');
                var endIndex = item.shejitimuContent.indexOf('>');
                    shejitimuContentCell.innerHTML = item.shejitimuContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
            } else {
                if (item.shejitimuContent != null && item.shejitimuContent != "" && item.shejitimuContent.length >= 11) {
                        shejitimuContentCell.innerHTML = item.shejitimuContent.substring(0, 10) + "...";
                } else {
                        shejitimuContentCell.innerHTML = item.shejitimuContent;
                }
            }
            row.appendChild(shejitimuContentCell);
            ...
            if(item.zhuangtaiTypes != 2){
                var detailBtn = document.createElement('button');
                var detailAttr = "fuzhi(" + item.id + ')';
                detailBtn.setAttribute("type", "button");
                detailBtn.setAttribute("class", "btn btn-primary btn-sm 投票");
                detailBtn.setAttribute("data-toggle", "modal");
                detailBtn.setAttribute("data-target", "#myModal");
                detailBtn.setAttribute("onclick", detailAttr);
                detailBtn.innerHTML = "投票";
                btnGroup.appendChild(detailBtn);
            }
            if(item.zhuangtaiTypes == 2) {
                ...
                editBtn.setAttribute("class", "btn btn-info btn-sm 选择项目");
                editBtn.setAttribute("onclick", editAttr);
                editBtn.innerHTML = "选择项目";
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
                ...
                httpJson("shejitimu/delete", "POST", paramArray, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('删除成功');
                    }
        ...
        // 投票
        function topiao(id) {
            var mymessage = confirm("真的要投票吗？");
            if (mymessage == true) {
                http("shejitimu/"+$("#toupiao").val()+"?ids="+$("#topiaoId").val(), "get", {}, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('操作成功');
                    $('#myModal').modal('hide')
        ...
        // 投票
        function xuanzhe(id) {
            var mymessage = confirm("真的要选择这个毕业设计项目吗？");
            if (mymessage == true) {
                http("shejitimu/xuanzhe?ids="+id, "get", {}, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('操作成功');
                    }
        ...
        function shenhe1(id) {
            ...
            if (mymessage == true) {
                http("shejitimu/shenhe?ids="+$("#shenheId").val()+"&zhuangtai="+$("#shenhe").val(), "get", {}, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('操作成功');
                    $('#myModal2').modal('hide')
        ...
        //添加
        function add() {
            window.sessionStorage.setItem("addshejitimu", "addshejitimu");
            window.location.href = "add-or-update.jsp"
        }
